...
class FloatingFragment(QWidget):
    ...
    def __init__(self, points, state, started_at, reminder_interval_minutes=10, on_click=None):
        super().__init__()
        self.points = [QPointF(p) for p in points]
        self.state = state
        self.started_at = started_at
    ...
    def _tick(self):
        ...
        interval_seconds = self.reminder_interval_minutes * 60
    ...
    def circle_left_open(self, state, started_at, points):
        ...
        if self.floating_fragment is None:
            self.floating_fragment = FloatingFragment(
                ...
                reminder_interval_minutes=self.config.get("fragment_reminder_interval", 10),
