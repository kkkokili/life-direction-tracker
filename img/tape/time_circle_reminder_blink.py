...
class FloatingFragment(QWidget):
    ...
    def __init__(self, points, state, started_at, on_click=None, reminder_minutes=10):
        super().__init__()
        self.points = [QPointF(p) for p in points]
        self.state = state
        self.started_at = started_at
        ...
        self.flash_steps_remaining = 0
        self.flash_on = False
        self.status_text = ""
        self.status_timer = QTimer(self)
        self.status_timer.setSingleShot(True)
        self.status_timer.timeout.connect(self.clear_status_text)

        self.flash_timer = QTimer(self)
        self.flash_timer.timeout.connect(self._flash_step)

        self.reminder_minutes = max(1, safe_int(reminder_minutes, 10))
        self.reminder_timer = QTimer(self)
        self.reminder_timer.timeout.connect(self.reminder_flash)
        ...
        self.set_reminder_minutes(self.reminder_minutes)
    ...
    def set_reminder_minutes(self, minutes):
        self.reminder_minutes = max(1, safe_int(minutes, 10))
        self.reminder_timer.stop()
        self.reminder_timer.start(self.reminder_minutes * 60 * 1000)
    ...
    def reminder_flash(self):
        # Regular gentle reminder: original color <-> white, three flashes.
        self.start_flash(times=3)

    def show_complete_hint(self):
        # Triggered when the restricted app is closed.
        self.status_text = "complete the circle"
        self.status_timer.start(4200)
        self.start_flash(times=2)

    def clear_status_text(self):
        self.status_text = ""
        self.update()

    def start_flash(self, times=3):
        self.flash_steps_remaining = max(1, times) * 2
        self.flash_on = False
        self.flash_timer.start(180)

    def _flash_step(self):
        if self.flash_steps_remaining <= 0:
    ...
    def paintEvent(self, event):
        ...
        if self.status_text:
            painter.setPen(QColor(104, 92, 80, 185))
            painter.setFont(QFont("Segoe UI", 8))
            painter.drawText(
                QRectF(12, 14, self.width() - 24, 18),
                Qt.AlignCenter,
                self.status_text
            )
...
class ReminderWindow(QWidget):
    ...
    def circle_left_open(self, state, started_at, points):
        ...
        if self.floating_fragment is None:
            ...
            self.floating_fragment.set_reminder_minutes(self.reminder_minutes)
    ...
    def notify_restricted_app_closed(self):
        if self.floating_fragment:
            self.floating_fragment.show_complete_hint()
...
class SettingsWindow(QWidget):
    def __init__(self, config, on_save_callback):
        ...
        self.reminder_input.setRange(1, 240)
        self.reminder_input.setSuffix(" min")
        self.reminder_input.setValue(
            max(1, safe_int(self.config.get("reminder_minutes"), 10))
        )
    ...
    def save(self):
        ...
        self.config["reminder_minutes"] = self.reminder_input.value()
...
class SleepGuardApp:
    def __init__(self):
        ...
        self.reminder.set_reminder_minutes(
            self.config.get("reminder_minutes", 10)
        )
    ...
    def check_restricted_apps(self):
        ...
        if self.last_triggered_app is not None:
            self.reminder.notify_restricted_app_closed()
